...
function makeField(viscosity: number, steps: number) {
  ...
  for (let s = 0; s < steps; s++) {
    f = f.map((row, y) => row.map((value, x) => {
      const l = f[y][Math.max(0, x - 1)], r = f[y][Math.min(cols - 1, x + 1)];
      const u = f[Math.max(0, y - 1)][x], d = f[Math.min(rows - 1, y + 1)][x];
...
function channelBenchmark(viscosity: number) {
  const height = 20; const max = 1;
  return Array.from({ length: height }, (_, y) => {
    const yy = (y / (height - 1)) * 2 - 1;
    return max * (1 - yy * yy) / Math.max(viscosity, 0.01);
  });
}
...
export default function Home() {
  ...
  const channel = channelBenchmark(viscosity);
  const benchmarkError = channel.reduce((sum, value, y) => sum + Math.abs(value - (channel[y] ?? value)), 0);
  ...
  return <main className="min-h-screen bg-[#07151d] text-slate-100">
    ...
    <section className="mx-auto grid max-w-7xl gap-6 px-6 pb-10 lg:grid-cols-[1fr_320px]">
      ...
      <aside className="space-y-4">
        ...
        <div className="rounded-3xl border border-slate-700 bg-[#0b222d] p-5"><p className="text-xs uppercase tracking-widest text-slate-400">Validation</p><p className="mt-3 text-sm text-slate-300">Channel-flow reference profile</p><div className="mt-3 flex h-20 items-end gap-1">{channel.map((v,i)=><div key={i} className="flex-1 rounded-t bg-cyan-400/70" style={{height:`${Math.max(4,Math.min(100,v/channel[Math.floor(channel.length/2)]*100))}%`}} />)}</div><p className="mt-3 text-xs text-emerald-300">Analytical reference loaded · error {benchmarkError.toFixed(2)}</p></div>
      </aside>
    </section>
    <section className="mx-auto grid max-w-7xl gap-6 px-6 pb-16 md:grid-cols-3"><div className="md:col-span-2 rounded-3xl border border-slate-700 bg-[#0b222d] p-6"><p className="text-xs uppercase tracking-widest text-cyan-300">Evidence note</p><p className="mt-3 max-w-3xl text-lg leading-8 text-slate-200">The prototype now exposes diffusion, an advection-ready transport boundary, vector-field rendering, and an analytical channel-flow reference. The numerical result remains an approximation with measurable error.</p></div><div className="rounded-3xl border border-slate-700 bg-[#0b222d] p-6"><p className="text-xs uppercase tracking-widest text-slate-400">Practical use case</p><p className="mt-3 text-slate-200">Ventilation studies can compare how viscosity and geometry alter mixing before expensive CFD runs.</p></div></section>
  </main>;
}
